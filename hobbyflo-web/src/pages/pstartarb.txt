...
const pstartarb: React.FC = () => {
  const connect = useConnect();

  const onStartArbitration = async () => {
   ...
   const escrowAmount = useContract.escrow();
    ...
    // Start arbitration
    await contract.startArbitration();
  };

  return (
    <button onClick={onStartArbitration}>Start Arbitration</button>
  );
};

export default pstartarb;